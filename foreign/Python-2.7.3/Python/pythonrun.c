...
Py_InitializeEx(int install_sigs)
{
    ...
    DBP("Initialized __builtin__ and exceptions\n");
                     ...
                     PyArena *arena)
{
    ...
    DBP("Attempting to parse file %s flags!\n", filename);
    ...
    DBP("checking if flags parsed correctly for %s!\n", filename);
    if (n) {
        flags->cf_flags |= iflags & PyCF_MASK;
        mod = PyAST_FromNode(n, flags, filename, arena);
        PyNode_Free(n);
